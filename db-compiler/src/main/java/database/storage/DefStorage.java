...
public class DefStorage {
    ...
    public static void updateIndex(final File table, final IColumnDef columnDef, final Object value, final Index indexObj, final int index) {
        indexObj.put(value, index);

        setIndex(table, columnDef, indexObj);
    }
    ...
    private static void bindIndex(final File table, final ITableDef tableDef, final IColumnDef columnDef, final Index index) {
        ...
        for (int i = 0; i < tableDef.getRowsCount(); i++) {
            Object key = records.getAsObject(columnDef);
            index.put(key, i);
            records.next();
        }
    }

    public static Index getIndex(final File table, final IColumnDef columnDef) {
        final String columnName = columnDef.getName();
        final File fileIndex = FileManager.getIndex(table, columnName);

        if (fileIndex != null) {
            try (FileInputStream input = new FileInputStream(fileIndex);
    ...
    private static void setIndex(final File table, final IColumnDef columnDef, final Index index) {
        final String columnName = columnDef.getName();
        final File fileIndex = FileManager.getIndex(table, columnName);

        try (FileOutputStream output = new FileOutputStream(fileIndex);
